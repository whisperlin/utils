...
namespace ShaderForge {
	...
	[System.Serializable]
	public class SFN_Tex2dAry : SF_Node {
		...
		public Texture TextureAsset {
			get {
				if(TexAssetConnected()){
					textureAsset = null;
					return ( GetInputCon( "2DAry" ).node as SFN_Tex2dAry ).textureAsset;
				}
				return textureAsset;
			}
			set {
		...
		public override void Initialize() {
			...
			connectors = new SF_NodeConnector[]{
				...
				SF_NodeConnector.Create(this,"2DAry","2DAry",ConType.cInput,ValueType.TexAsset).WithColor(SF_Node.colorExposed),
		...
		public bool IsNormalMap() {
			...
			if(TexAssetConnected())
				return ( GetInputCon( "2DAry" ).node as SFN_Tex2dAry ).IsNormalMap();
		...
		public override string Evaluate( OutChannel channel = OutChannel.All ) {
			...
			bool useLOD = ( SF_Evaluator.inVert || SF_Evaluator.inTess );
			...
			string mip = "0";
			string variableName = this["2DAry"].IsConnected() ? GetInputCon( "2DAry" ).node.property.GetVariable() : property.GetVariable();

			bool useTilingLocally = IsProperty() && !property.tagNoScaleOffset;
			bool useTilingByAsset = this["2DAry"].IsConnected() && !this["2DAry"].inputCon.node.property.tagNoScaleOffset;
		...
		public override bool Draw() {
			...
			if( property == null ) {
				...
				SFN_Tex2dAry inTex = ( GetInputCon( "2DAry" ).node as SFN_Tex2dAry );
		...
		public override void NeatWindow(  ) {
			...
			rect.height = (TexAssetConnected() ? NODE_HEIGHT : NODE_HEIGHT + 34)+40;
