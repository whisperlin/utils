...
namespace ShaderForge {
	...
	public class SF_Evaluator {
		...
		void BeginCG() {
			...
			//billboard.
			if (ps.catGeometry.billboard == BILLBOARD_TYPE.Y_AXIS) {
			
				App( @"void BillboardY(inout float4 vertex,inout float3 normal )
				{
			...
			else if (ps.catGeometry.billboard == BILLBOARD_TYPE.ALL) {

				App( @"void Billboard(inout float4 vertex,inout float3 normal )
				{
					const float3 local = float3(vertex.x, vertex.y, 0); // this is the quad verts as generated by MakeMesh.cs in the localPos list.
					const float3 offset = vertex.xyz - local;
		...
		*/
		...
		void CommonVertexData() {
			//billboard.
			if( dependencies.vert_in_normals || ps.catGeometry.billboard != BILLBOARD_TYPE.NONE)
		...
		void Vertex() {
			...
			if (ps.catGeometry.billboard == BILLBOARD_TYPE.Y_AXIS) {
				...
				App( @"BillboardY(v.vertex,v.normal);");

			}
			else if (ps.catGeometry.billboard == BILLBOARD_TYPE.ALL) {

				App( @"Billboard(v.vertex,v.normal);");
			}
			if( !editor.nodeView.treeStatus.viewDirectionInVertOffset ) {
				InitViewDirVert();
			}
