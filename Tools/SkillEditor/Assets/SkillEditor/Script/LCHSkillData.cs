...
public class LCHObjectData
{
    ...
    public int type = 1;//1物体，2触发器 3，音效
        ...
        for (int j = 0 ; j < ct0; j++)
        {
            ...
            else if (t > time)
            {
                ...
                else
                {
                    float t0 = time - times[j - 1];
                    float t1 = t - times[j - 1];
                    return  Mathf.Lerp(values[j - 1], values[j], t0/t1);
                }
            }
        }
        if(ct0>0)
...
public class LCHEventChannelData
{
    ...
    public bool TryGetKeyFrameRunntimme(float curFrame, out ObjDictionary value, out float time)
    {
