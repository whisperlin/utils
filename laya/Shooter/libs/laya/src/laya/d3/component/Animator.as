package laya.d3.component {
	...
	public class Animator extends Component {
		...
		public var scriptRatation:Quaternion;
		public var scriptRotationNodeIndex : int;
		...
		public function Animator() {
			...
			scriptRatation = new Quaternion();
			scriptRotationNodeIndex = -1;
		}
		...
		private function _applyRotationEx(nodeOwner:KeyframeNodeOwner, additive:Boolean, weight:Number, isFirstLayer:Boolean, clipRot:Float32Array, localRotationE:Float32Array):void {
			
	 
			//Quaternion.createFromYawPitchRoll(scriptRatation.x, scriptRatation.y ,scriptRatation.z, localRotation);
			var oe:Float32Array = scriptRatation.elements;
			localRotationE[0] = oe[0];
			localRotationE[1] = oe[1];
			localRotationE[2] = oe[2];
			localRotationE[3] = oe[3];

			//console.log(scriptRatation.elements);
		}
				...
				if (nodeOwner) {//骨骼中没有该节点
					...
					if (pro) {
						...
						case 2: //Rotation
							...
							if(i==scriptRotationNodeIndex)
							{
								_applyRotationEx(nodeOwner, additive, weight, isFirstLayer, nodes.getNodeByIndex(i).data, localRot.elements);
							}
							else
							{
		...
		public function getlayer(index:int = 0):AnimatorControllerLayer
		{
			var controllerLayer:AnimatorControllerLayer = _controllerLayers[index];
