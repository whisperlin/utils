...
namespace ShaderForge {
	...
	public class SF_Evaluator {
        ...
        public bool HasRich5VirtualLights()
        {
            for(int i = 0, c = cNodes.Count; i < c; i++)
            {
                SF_Node node = cNodes[i];
                if (node is SFN_VirtualLightsDiffuse)
                    return true;
            }

            return false;
        }
		public bool HasPBRLodNode()
		{
			for (int i = 0, c = cNodes.Count; i < c; i++)
			{
				SF_Node node = cNodes[i];
				if (node is SFN_PBRLod || node is SFN_PBRLod2)
					return true;
			}

			return false;
		}
        public bool HasRich5VirtualSpecular()
        {
            for (int i = 0, c = cNodes.Count; i < c; i++)
            {
                SF_Node node = cNodes[i];
                if (node is SFN_Specular)
                    return true;
            }
        ...
        public void UpdateDependencies() {
            ...
            if (HasRich5VirtualLights())
            {
                ...
                dependencies.NeedVertWorldPos();
            }
		 
			if (HasRich5VirtualSpecular() || HasPBRLodNode() )
            {
            ...
            if (ps.HasRich5ShadowUsed())
            {
                ...
                dependencies.NeedFragWorldPos();
                dependencies.NeedVertWorldPos();
                dependencies.vert_out_shadow_viewpos = true;
            }
            ///------------------------------------
		...
		void BeginCG() {
            ...
            //------rich5 ×¨ÓÃshader
            if (ps.HasRich5Output())
            {
                App("#pragma multi_compile __ AlphaBlendOn UseUnityShadow");
                App("#pragma multi_compile_instancing");
                //App("#pragma multi_compile_fwdbase_fullshadows")
                App("#include \"AutoLight.cginc\"");
                App("#include \"../CGIncludes/DiffuseInfo.cginc\"");

                App("\n");
                App("INSTANCING_START");
                App("INSTANCING_PROP_UVST");
                App("INSTANCING_END");
                App("\n");
            }
		...
		*/
		...
		void Lighting() {
			...
			*/
			...
			if( !ps.catLighting.IsLit() && SF_Evaluator.inFrag ) {
                ...
                bool doRich5Ount = ps.HasRich5Output();
                bool doRich5_Shadow_input = ps.HasRich5ShadowUsed();

				string s = "float3 finalColor = ";
                if (doRich5_Shadow_input)
                    s = "float4 finalColor = ";
				...
				if( !didAddLight && !doRich5Ount)
					s += "0"; // TODO: Don't do lighting at all if this is the case


				s += ";";

                if (doRich5_Shadow_input && doRich5Ount)
                {
                    s += ("\nfinalColor.a *= " + ps.n_alpha + ";");
                }                    
		...
		void VertexOutputStruct() {
			...
			scope++;
			{
                ...
                //-------------------------dependencies.vert_out_shadow_viewpos---test-
                if (dependencies.vert_out_shadow_viewpos)
                {
                    App("float4 viewPos" + GetVertOutTexcoord());
                    App("float4 worldPos_2_Camera" + GetVertOutTexcoord()); 
                }
            ...
            {
                App("UNITY_VERTEX_INPUT_INSTANCE_ID\n");
                App("#ifdef UseUnityShadow");
                App(string.Format("LIGHTING_COORDS({0}, {1})", dependencies.GetVertOutTexcoord(), dependencies.GetVertOutTexcoord()));
                App("#endif");
            }
            ...
            if (dependencies.vert_out_shadow_viewpos)
            {
                App("#include \"../CGIncludes/shadow.cginc\"");
            }
		}
		...
		void Vertex() {
            ...
            //-------------------------dependencies.vert_out_shadow_viewpos---test-
            if (dependencies.vert_out_shadow_viewpos)
                App("GET_CAMERA_POS_NORMAL(o.posWorld, o.viewPos, o.worldPos_2_Camera, o.normalDir);");
            //--------------------------
		...
		void Fragment() {
			...
			if( currentPass == PassType.Meta ) {
                ...
                else
                {
					...
					if( ps.catBlending.alphaToCoverage && 
                        currentPass == PassType.FwdBase && 
                        ps.HasAlpha() == false && ps.HasAlphaClip() )
                    {
						AppFinalOutput( "finalColor", "(" + ps.n_alphaClip + ") * 2.0 - 1.0" );
					}
                    else if (ps.HasRich5ShadowUsed())
                    {
                        if (ps.catBlending.fogOverrideColor)
                        {
                            App("UNITY_APPLY_FOG_COLOR(i.fogCoord, finalColor.rgb, " + GetFogColorAsFixed3Value() + ");");
                        }
                        else
                        {
                            App("UNITY_APPLY_FOG(i.fogCoord, finalColor.rgb);");
                        }

                        App("return finalColor;");
                    }
                    else
                    {
