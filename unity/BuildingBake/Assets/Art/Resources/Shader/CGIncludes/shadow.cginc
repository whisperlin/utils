...
float4 FINAL_SHADOW_COLOR_SINGLE(float3 col, VertexOutput i, float3 normal ) 
{
	...
	return float4((col * EnvimentColor * (NdotL > 0 ? LIGHT_ATTENUATION(i) : 1)), ShadowAlpha);
}
...
float4 FINAL_SHADOW_COLOR_SINGLE(float3 col, VertexOutput i, float3 normal) 
{
	...
	return FINAL_SHADOW_COLOR(float4(col * EnvimentColor, 1), Depth, absZ, NdotL, ShadowAlpha );
}
#endif
