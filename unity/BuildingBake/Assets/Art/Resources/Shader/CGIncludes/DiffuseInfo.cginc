	...
	float3 _MainTex_var, float3 _DiffCubemap, float3 _SpecCubemap)
{
	...
	float3 indirectSpecular = (0 + _SpecCubemap.rgb  *_Metallic_red);
	...
	float negative = step(ndotl, 0);
	float ndotl_abs = abs(ndotl);
	float3  newdiff = (  directDiffuse + _DiffCubemap.rgb + NegativeLightColor0 * NegativeLightIntensity * ndotl_abs * negative)  *diffuseColor ;
...
inline float3 PBRSpecularLow(float3 normalDirection, float3 viewDirection,float3 _Metallic_var,float _Metallic_alpha,float _Metallic_red,
	float3 _MainTex_var,  float3 _DiffCubemap, float3 _SpecColor)
{
    ...
    float lhPow3 = Pow3(1-LdotH);
 	float specularPBL =   NdotL *roughness     ; 

    float f0 = (specularColor + (1-specularColor) * lhPow3) *specularPBL; 
	float3 indirectSpecular = (0 + _SpecColor.rgb  *_Metallic_red);                
    indirectSpecular =   _SpecColor.rgb*_Metallic_red  *pow(NdotH, 3) ;

    float3 directSpecular = DirectionLightColor0*DirectionLightIntensity0*f0; 
	...
	float3  newdiff = (  directDiffuse  + indirectDiffuse)  *diffuseColor ;
	 ...
	 float3 specular = (directSpecular + indirectSpecular*newdiff);
	return  newdiff  + specular ;
}
