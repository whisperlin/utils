...
float LightRange3;
float LightRange4;
inline float3 PointLight2(float3 normaldir,float3 p){
...
inline float3 PointLight(float3 normaldir,float3 p,float LightRange){
	float3 lightDirection;
	float atten;
   float3 lightvector = p - posWorld.xyz;
   float distance = length(lightvector);
    
    //if ( distance > LightRange3 )
    //	return 0;
    //else
    //	return 1;
    atten = 1.0 - distance / LightRange;
    atten = max (0,atten);
    lightDirection = normalize(lightvector);
	float3 finalPointlight = atten * saturate(dot(normaldir,lightDirection));
    return finalPointlight;
    ...
    float3 pos, float3 normal)
{
    ...
    float4 toLightY = lightPosX - pos.y;
    float4 toLightZ = lightPosX - pos.z;
    // squared lengths
    float4 lengthSq = 0;
