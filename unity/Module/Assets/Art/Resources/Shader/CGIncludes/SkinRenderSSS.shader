		...
		SubShader
		{
			...
			Pass
			{
				...
				v2f vert(appdata v)
				{
					...
					//o.vertex = UnityObjectToClipPos(v.vertex);
					o.uv = v.uv;
					o.uv1 = TRANSFORM_TEX(v.uv, _Metallic);
...
float _Roughness;
		...
		fixed4 frag(v2f i): SV_Target
		{
			...
			float3 Nx =i.normalDir;
			float3 N = normalize(mul(normalLocal, tangentTransform)); // Perturbed normal
			...
			fixed4 AlbedoColor = tex2D(_MainTex, TRANSFORM_TEX(i.uv, _MainTex))* _MainTexTin;
			...
			float curvature = normalize (   length(fwidth((normalize(N)))) /
				length(fwidth (i.posWorld))) * _CurveScale;
 
			float3 brdf = tex2D(_BRDFTex, float2((NdotL * 0.5 + 0.5), curvature)).rgb;
