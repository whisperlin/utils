...
inline half SmithJointGGXVisibilityTerm(half NdL, half NdV, half k)
{
	half gL = NdL * (1 - k) + k;
	half gV = NdV * (1 - k) + k;
...
//捏菲尔算法
inline half3 FresnelTerm(half3 specularColor, half LdotH)
{
...
// H = normalize(L+V) 
// F = (N·H)*s
float3 BlinnPhong(float3 LightDir,float3 normalDir,float3 MetallicTex){
	...
	float specPow = exp2(_GlossPower * 10 + 1 ) ;  
	float3 fianl =  max(0 ,pow(F,specPow)*specPow * MetallicTex *_MetallicPower  )  ;
	return fianl;
}
 */
	          ...
	          float4 MetallicTex, float3 diffuseColor ,out float3 diffusefinalColor)
{
 ...
 // return float4(diffusefinalColor,.);
  ...
  //  diffuseColor = DiffuseAndSpecularFromMetallic(diffuseColor, specularColor, specularColor, specularMonochrome );
    ...
    specularPBL = (visTerm*normTerm) * UNITY_PI;
...
inline float3 DirectionalLight1(float3 NorDir) {      
    float3 dir1 = max(0.0,dot( NorDir, -LightDir1 ));
    return dir1 * LightIntensity1 * LightColor1 ;
}
...
inline fixed3 SpecularD(fixed3 n,fixed3 h,half Roughness,float3 normalDir,float3 LightDir){
   ...
   fixed3 d = Pow2(PI*(Pow2(dot(n,h)) * (u - 1) + 1 ));
   return u/d;
}
