...
Shader "Physically-Based-Lighting" {
	...
	[Toggle] _ENABLE_NDF ("D NFD Normal Distribution Enabled?", Float) = 0
	[Toggle] _ENABLE_G ("G Geometric Shadow Enabled?", Float) = 0
...
float TrowbridgeReitzAnisotropicNormalDistribution(float anisotropic, float NdotH, float HdotT, float HdotB){
...
float WardAnisotropicNormalDistribution(float anisotropic, float NdotL, float NdotV, float NdotH, float HdotT, float HdotB){
    float aspect = sqrt(1.0h-anisotropic * 0.9h);
    float X = max(.001, sqr(1.0-_Glossiness)/aspect) * 5;
 	float Y = max(.001, sqr(1.0-_Glossiness)*aspect) * 5;
...
//light calculations
    ...
    #endif
	#ifdef _ENABLE_D_ON 
 	 return float4(float3(1,1,1)* diffuseColor*NdotL,1);
    #endif
