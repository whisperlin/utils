...
#include "ue4.cginc"
	...
	float3 atten = _LightColor0.rgb*NdotL;
	...
	float specularPBL = ArmBRDF(roughness, NdotH, LdotH)*specColor;
	...
	//float3 spe = specOcclusion * SpecularIBL(specColor, roughness, worldNormal, viewDir);
	float3 spe = ApproximateSpecularIBL(specColor, roughness, worldNormal, viewDir, _Lut);

	//float3 spe = specOcclusion *specColor* PrefilterEnvMap(roughness, viewReflect);
	//return float4(spe,1);
	return float4(emissive + diffColor*diffuseShading + specularPBL+spe,1);
	}
		ENDCG
	}
	}
