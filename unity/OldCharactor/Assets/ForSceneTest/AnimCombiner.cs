...
public class AnimCombiner : MonoBehaviour {
	...
	static void CombineMeshs(SkinnedMeshRenderer[] anims, MeshRenderer[] meshs, Animation root) {
		...
		for(int i = 0; i < anims.Length; i++) {
			...
			var anim_root_bone = anims[i].rootBone;
			if(anim_root_bone) {
				if(root_bone == null || root_bone.IsChildOf(anim_root_bone)) {
					root_bone = anims[i].rootBone;
				}
			}
		}
		...
		if(mats.Count == 1) {
			...
			if(mat.shader.name.Equals("YuLongZhi/Character")) {
			...
			} else if(mat.shader.name.Equals("YuLongZhi/CharacterPBR")) {
				mat.shader = Shader.Find("YuLongZhi/CharacterPBRCombine");
				mat.SetTexture("_MainTex2", mats[1].GetTexture("_MainTex"));
				mat.SetTexture("_Normal2", mats[1].GetTexture("_Normal"));
				mat.SetTexture("_SpecRefSmoothnessEmissive2", mats[1].GetTexture("_SpecRefSmoothnessEmissive"));
			} else {
				Debug.LogError("anim combine not support this shader ");
			}
		} else {
