...
public class T4MObjSC : MonoBehaviour {
    ...
    public string mBrushSavePath = "";

    public int MaskWidth = 64;
    public int MaskHeight = 64;

    Renderer mRenderer = null;
    MaterialPropertyBlock mMaterialBlock = null;
    int mTextureSizeHash = 0;
    public void Awake()
	{
		if (Master ==1){
	...
	void LateUpdate () {
        ...
        if (mMaterialBlock == null)
        {
            mRenderer = GetComponent<Renderer>();
            mMaterialBlock = new MaterialPropertyBlock();
            mRenderer.GetPropertyBlock(mMaterialBlock);
            mTextureSizeHash = Shader.PropertyToID("TextureSize");
        }

        mMaterialBlock.SetFloat(mTextureSizeHash, MaskWidth);
        mRenderer.SetPropertyBlock(mMaterialBlock);
