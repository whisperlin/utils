...
public class T4MSC : EditorWindow
{
    ...
    GUIContent[] MenuIcon = new GUIContent[8];

    static public int T4MMenuToolbar = 0;

    static public int CotnrolTextureSize = 1024;
    ...
    const int Texturelayer = 12;

    Texture []Texture_Layers = new Texture2D[Texturelayer];
    ...
    Vector2[] LayerTiles = new Vector2[Texturelayer];
//     Vector2 Layer1Tile;
//     Vector2 Layer2Tile;
//     Vector2 Layer3Tile;
//     Vector2 Layer4Tile;
...
//     Vector2 Layer8Tile;
    ...
    void OnGUI()
    {
		...
		MenuIcon[7] = new GUIContent(AssetDatabase.LoadAssetAtPath(T4MEditorFolder + "Icons/normal.png", typeof(Texture2D)) as Texture);
        ...
        T4MMenuToolbar = (int)GUILayout.Toolbar(T4MMenuToolbar, MenuIcon, "gridlist", GUILayout.Width(172+40), GUILayout.Height(18));
        ...
        if (CurrentSelect != null && T4MActived == "Activated")
        {
			...
			T4MObjSC sc = CurrentSelect.GetComponent<T4MObjSC> ();
			if (sc&& (!sc.T4MMaterial || !sc.T4MMesh))
            {
	...
	void NormalMenu()
	{
		T4MObjSC _c = CurrentSelect.GetComponent<T4MObjSC> ();
		if (_c) {
			...
			Texture2D [] normals = new Texture2D[8];
			Texture2D [] bases = new Texture2D[8];
			for (int i = 0; i < 8; i++) {
				string _param = "_Splat" + i.ToString ();
				if (_c.T4MMaterial.HasProperty ( _param ) ) {
					bases [i] = (Texture2D)_c.T4MMaterial.GetTexture (_param);
					string path = AssetDatabase.GetAssetPath(bases[i].GetInstanceID() )  ;
					path = path.Substring (0, path.Length - 4 ) + "_N.png";
					normals[i] =AssetDatabase.LoadAssetAtPath<Texture2D> (path);
					if(normals[i]==null)
					{
			...
			for (int i = 0; i < normals.Length ; i++) {
				EditorGUILayout.ObjectField ("_Splat"+i.ToString()+"法线" , normals [i],typeof(Texture2D),false);
			}

			//ial.GetTexture("_Splat0")) as Texture; 
			if (GUILayout.Button ("保存")) {
				for (int i = 1; i < 8; i++) {
					if (bases [i] == null)
						continue;
					if(bases[i].width != bases[i-1].width && bases[i].height != bases[i-1].height)
					{
						EditorUtility.DisplayDialog ("贴图", "有贴图的宽高不一样"+bases[i-1].name+" "+bases[i].name,"确定" );
					}
					if(bases[i].format != bases[i-1].format)
					{
						EditorUtility.DisplayDialog ("贴图", "有贴图的格式不一样"+bases[i-1].name+" "+bases[i-1].format.ToString()+" "+bases[i].name + " "+bases[i].format.ToString() ,"确定");
					}
				}
				for (int i = 1; i < 8; i++) {
					if (normals [i] == null)
						continue;
					if(normals[i].width != normals[i-1].width && normals[i].height != normals[i-1].height)
					{
						EditorUtility.DisplayDialog ("法线贴图", "有法线贴图的宽高不一样"+normals[i-1].name+" "+normals[i].name,"确定" );
					}
					if(normals[i].format != normals[i-1].format)
					{
						EditorUtility.DisplayDialog ("法线贴图", "有法线贴图的格式不一样"+normals[i-1].name+" "+normals[i-1].format.ToString()+" "+normals[i].name + " "+normals[i].format.ToString() ,"确定");
					}
				}
				//layerTextures[0].format

				Texture2D Controller1 = _c.T4MMaterial.GetTexture("_Control") as Texture2D;
				...
				string path1 = TexturePath.Replace(fileName, "TextureArray.asset");

				//TextureArray.asset.
				//bases
				Texture2DArray basesArray = TerrainMaker.CreateTextureArray2D(bases, path1);
				Texture2DArray normalArray = TerrainMaker.CreateTextureArray2D(normals, path);

				_c.T4MMaterial.SetTexture("_Splat_Normals", normalArray);
			}
        ...
        if (CurrentSelect.GetComponent<T4MObjSC>())
        {
            if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial && CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_Splat0") && CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_Splat1") && CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_Control"))
            {
                ...
                if (intialized)
                {
                    ...
                    GUILayout.FlexibleSpace();
                    if (TexTexture.Length > 8)
                        T4MselTexture = GUILayout.SelectionGrid(T4MselTexture, TexTexture, 12, "gridlist", GUILayout.Width(340), GUILayout.Height(35));
                    else if (TexTexture.Length > 4)
                        T4MselTexture = GUILayout.SelectionGrid(T4MselTexture, TexTexture, 8, "gridlist", GUILayout.Width(340), GUILayout.Height(58));
                    else
                    ...
                    if (joinTiles)
                    {
                        for(int i=0;i< TexTexture.Length; i++)
                        {
                            string Splat_layer = "_Splat" + i;
                            EditorGUILayout.Space();
                            if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty(Splat_layer))
                            {
                                LayerTiles[i].x = LayerTiles[i].y = EditorGUILayout.Slider(Splat_layer +"Tiling :", LayerTiles[i].x, 1, 500 * T4MMaskTexUVCoord);
                                CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.SetTextureScale(Splat_layer, new Vector2(LayerTiles[i].x, LayerTiles[i].x));
                            }
                            EditorGUILayout.Space();
                        }
#if false
                        ...
                        if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_Splat6"))
                        {
                            Layer7Tile.x = Layer7Tile.y = EditorGUILayout.Slider("Layer7 Tiling :", Layer7Tile.x, 1, 500 * T4MMaskTexUVCoord);
                            CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.SetTextureScale("_Splat6", new Vector2(Layer7Tile.x, Layer7Tile.x));
                            CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.SetVector("_Tiling7", new Vector4(Layer7Tile.x, Layer7Tile.x, 0, 0));
                        }

                        if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_Splat7"))
                        {
                            Layer8Tile.x = Layer8Tile.y = EditorGUILayout.Slider("Layer8 Tiling :", Layer8Tile.x, 1, 500 * T4MMaskTexUVCoord);
                            CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.SetTextureScale("_Splat7", new Vector2(Layer8Tile.x, Layer8Tile.x));
                            CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.SetVector("_Tiling7", new Vector4(Layer8Tile.x, Layer8Tile.x, 0, 0));
                        }
#endif
                    }
                    else
                    {
                        for(int i=0;i<TexTexture.Length;i++)
                        {
                            ...
                            if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty(Splat_layer))
                            {
                                LayerTiles[i].x = EditorGUILayout.Slider(Splat_layer+" TilingX :", LayerTiles[i].x, 1, 500 * T4MMaskTexUVCoord);
                                LayerTiles[i].y = EditorGUILayout.Slider(Splat_layer+" TilingZ :", LayerTiles[i].y, 1, 500 * T4MMaskTexUVCoord);
                                CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.SetTextureScale(Splat_layer, new Vector2(LayerTiles[i].x, LayerTiles[i].y));
                            }
                            EditorGUILayout.Space();
                        }
#if false
                        ...
                        if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_Splat6"))
                        {
                            Layer6Tile.x = EditorGUILayout.Slider("Layer7 TilingX :", Layer7Tile.x, 1, 500 * T4MMaskTexUVCoord);
                            Layer6Tile.y = EditorGUILayout.Slider("Layer7 TilingZ :", Layer7Tile.y, 1, 500 * T4MMaskTexUVCoord);
                            CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.SetTextureScale("_Splat6", new Vector2(Layer7Tile.x, Layer7Tile.y));
                            CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.SetVector("_Tiling6", new Vector4(Layer7Tile.x, Layer7Tile.y, 0, 0));
                        }

                        EditorGUILayout.Space();
                        if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_Splat7"))
                        {
                            Layer6Tile.x = EditorGUILayout.Slider("Layer8 TilingX :", Layer8Tile.x, 1, 500 * T4MMaskTexUVCoord);
                            Layer6Tile.y = EditorGUILayout.Slider("Layer8 TilingZ :", Layer8Tile.y, 1, 500 * T4MMaskTexUVCoord);
                            CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.SetTextureScale("_Splat7", new Vector2(Layer8Tile.x, Layer8Tile.y));
                            CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.SetVector("_Tiling7", new Vector4(Layer8Tile.x, Layer8Tile.y, 0, 0));
                        }
#endif
                    }
                    ...
                    if (TexBrush.Length > 0)
                    {
                        ...
                        T4MPreview.orthographicSize = (brushSize * CurrentSelect.localScale.x) * (temp.sharedMesh.bounds.size.x / 2000);
                    ...
                    T4MBrushSizeInPourcent = (int)Mathf.Round((brushSize * T4MMaskTex.width) / 1000);

                    for(int i=0;i<Texturelayer;i++)
                    {
                        if (T4MselTexture == i)
                        {
                            T4MPreview.material.SetTextureScale("_MainTex", LayerTiles[i]);
                            T4MPreview.material.SetVector("_Tiling", 0.1f * new Vector4(LayerTiles[i].x, LayerTiles[i].y, 0, 0));
                            break;
                        }
                    }
                    ...
                    else if (T4MselTexture == 6)
                    {
                        ...
                        T4MPreview.material.SetVector("_Tiling", 0.1f * new Vector4(Layer7Tile.x, Layer7Tile.y, 0, 0));
                    }
                    else if (T4MselTexture == 7)
                    {
                        T4MPreview.material.SetTextureScale("_MainTex", Layer8Tile);
                        T4MPreview.material.SetVector("_Tiling", 0.1f * new Vector4(Layer8Tile.x, Layer8Tile.y, 0, 0));
                    }
#endif
                    if (selBrush != oldSelBrush || T4MBrushSizeInPourcent != oldBrushSizeInPourcent || T4MBrushAlpha == null || T4MselTexture != oldselTexture)
                    {
                        ...
                        {
                            T4MPreview.material.SetTexture("_MainTex", CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.GetTexture("_Splat8") as Texture);
                            T4MtargetColor = new Color(0, 0, 0, 0);
                            if (T4MMaskTex2)
                                T4MtargetColor2 = new Color(0, 0, 0, 0);
                            if (T4MMaskTex3)
                        ...
                        {
                            T4MPreview.material.SetTexture("_MainTex", CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.GetTexture("_Splat9") as Texture);
                            T4MtargetColor = new Color(0, 0, 0, 0);
                            if (T4MMaskTex2)
                                T4MtargetColor2 = new Color(0, 0, 0, 0);
                            if (T4MMaskTex3)
                        ...
                        {
                            T4MPreview.material.SetTexture("_MainTex", CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.GetTexture("_Splat10") as Texture);
                            T4MtargetColor = new Color(0, 0, 0, 0);
                            if (T4MMaskTex2)
                                T4MtargetColor2 = new Color(0, 0, 0, 0);
                            if (T4MMaskTex3)
                        ...
                        {
                            T4MPreview.material.SetTexture("_MainTex", CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.GetTexture("_Splat11") as Texture);
                            T4MtargetColor = new Color(0, 0, 0, 0);
                            if (T4MMaskTex2)
                                T4MtargetColor2 = new Color(0, 0, 0, 0);
                            if (T4MMaskTex3)
    ...
    void InitPreview()
    {
        ...
        for(int i=0;i<Texturelayer;i++)
        {
            if (T4MselTexture == i)
            {
                ...
                T4MPreview.material.SetTextureScale("_MainTex", LayerTiles[i]);
                T4MPreview.material.SetVector("_Tiling", 0.1f * new Vector4(LayerTiles[i].x, LayerTiles[i].y, 0, 0));
                T4MPreview.material.SetTexture("_MainTex", CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.GetTexture(splat) as Texture);
                break;
            }
        }
    ...
    static public void SaveTexture()
    {
        ...
        if (T4MMaskTex3)
        {
            var path3 = AssetDatabase.GetAssetPath(T4MMaskTex3);
            var bytes3 = T4MMaskTex3.EncodeToPNG();
    ...
    void InitPincil()
    {
        if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_Splat11"))
        {
            TexTexture = new Texture[12];
        ...
        else if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_Splat10"))
        {
            TexTexture = new Texture[11];
            TexTexture[0] = AssetPreview.GetAssetPreview(CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.GetTexture("_Splat0")) as Texture;
            TexTexture[1] = AssetPreview.GetAssetPreview(CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.GetTexture("_Splat1")) as Texture;
            TexTexture[2] = AssetPreview.GetAssetPreview(CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.GetTexture("_Splat2")) as Texture;
            TexTexture[3] = AssetPreview.GetAssetPreview(CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.GetTexture("_Splat3")) as Texture;
        ...
        else if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_Splat9"))
        {
            TexTexture = new Texture[10];
            TexTexture[0] = AssetPreview.GetAssetPreview(CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.GetTexture("_Splat0")) as Texture;
            TexTexture[1] = AssetPreview.GetAssetPreview(CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.GetTexture("_Splat1")) as Texture;
            TexTexture[2] = AssetPreview.GetAssetPreview(CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.GetTexture("_Splat2")) as Texture;
            TexTexture[3] = AssetPreview.GetAssetPreview(CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.GetTexture("_Splat3")) as Texture;
        ...
        else if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_Splat8"))
        {
        ...
        else if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_Splat7"))
        {
	...
	void SaveCtrlTexture(string filespath , int size ,Color b )
	{
		...
		var bys = tex.EncodeToPNG();//转换图片资源.
		File.WriteAllBytes(filespath, bys);//保存图片到写好的目录下.
		 
	}
	void MovifyCtrlTexture(string filespath )
	{
		TextureImporter texture = AssetImporter.GetAtPath(filespath) as TextureImporter;
		texture.mipmapEnabled = false;
		texture.textureFormat = TextureImporterFormat.ARGB32;
		texture.textureCompression = TextureImporterCompression.Uncompressed;
		texture.isReadable = true; 
	
		AssetDatabase.ImportAsset(filespath);
		AssetDatabase.LoadAssetAtPath<Texture2D> (filespath);
	}
        ...
        if (vertexInfo == 0 && trisInfo == 0 && partofT4MObj == 0)
        {
            ...
            if ((CurrentSelect.GetComponent<Renderer>() || CurrentSelect.GetComponent<Terrain>() || NbrPartObj != null && NbrPartObj.Length != 0) && !CurrentSelect.GetComponent<T4MObjSC>() && !CurrentSelect.GetComponent<T4MPartSC>())
            {
                ...
                if (CurrentSelect.GetComponent<Terrain>())
                {
                    ...
                    GUILayout.BeginHorizontal();
                    GUILayout.FlexibleSpace();
                    T4MResolution = (int)GUILayout.HorizontalScrollbar(T4MResolution, 0, 32, 1024, GUILayout.Width(350));
                    GUILayout.FlexibleSpace();
                    GUILayout.EndHorizontal();
                    EditorGUILayout.Space();
                    EditorGUILayout.Space();
            ...
            else
            {
                ...
                GUILayout.BeginHorizontal();
                GUILayout.FlexibleSpace();
				T4MObjSC _t4m = CurrentSelect.GetComponent<T4MObjSC> ();
				if (_t4m)
				...
				if (GUILayout.Button ("设置shader")) {
					_t4m.T4MMaterial.shader = Shader.Find ( "DAFUHAO_Editor/T4M_12" );
				}
				if (GUILayout.Button ("检查并创建两张control贴图")) {
					
					Scene scene = SceneManager.GetActiveScene();
					if(scene.path.Length == 0)
					{
						EditorUtility.DisplayDialog ("提示", "请先保存场景", "ok");
						return;
					}
					string curPath = scene.path.Substring (  0 , scene.path.Length - 6 - scene.name.Length) + "textures";
					if(!System.IO.File.Exists(curPath))
						System.IO.Directory.CreateDirectory (curPath);
					//if (_t4m.T4MMaterial.GetTexture ("_Control") == null ) 
					{
						string filespath = curPath+@"/default1.png";
						if (!File.Exists (filespath)) {
							SaveCtrlTexture (filespath,1024,new Color (0, 0, 0, 0));
						} 
						AssetDatabase.ImportAsset (filespath);
						AssetDatabase.Refresh ();
						Texture2D t =  AssetDatabase.LoadAssetAtPath<Texture2D> (filespath);
						_t4m.T4MMaterial.SetTexture ("_Control",t);
						_t4m.T4MMaterial.SetColor ("_color",Color.white);
				 
						MovifyCtrlTexture (filespath);
					}
					//if (_t4m.T4MMaterial.GetTexture ("_Control2") == null ) 
					{
						string filespath = curPath+@"/default2.png";
						if (!File.Exists (filespath)) {
							SaveCtrlTexture (filespath,1024, new Color (1, 0, 0, 0));
						} 
						AssetDatabase.ImportAsset (filespath);
						AssetDatabase.Refresh ();
						Texture2D t =  AssetDatabase.LoadAssetAtPath<Texture2D> (filespath);
						_t4m.T4MMaterial.SetTexture ("_Control2",t);
						_t4m.T4MMaterial.SetColor ("_color",Color.white);
						MovifyCtrlTexture (filespath);
					}
            ...
            switch (MyT4MV)
            {
                ...
                case 1:
                    if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial && CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_Splat0") && CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_Splat1") && CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_Control"))
                    {
                        ...
                        GUILayout.FlexibleSpace();
                        if (TexTexture.Length > 8)
                            selProcedural = GUILayout.SelectionGrid(selProcedural, TexTexture, 12, "gridlist", GUILayout.Width(340), GUILayout.Height(30));
                        else if (TexTexture.Length > 4)
                            selProcedural = GUILayout.SelectionGrid(selProcedural, TexTexture, 8, "gridlist", GUILayout.Width(340), GUILayout.Height(58));
                        else
                            selProcedural = GUILayout.SelectionGrid(selProcedural, TexTexture, 4, "gridlist", GUILayout.Width(340), GUILayout.Height(80));
                        GUILayout.FlexibleSpace();
                        GUILayout.EndHorizontal();
                        GUILayout.FlexibleSpace();
                        GUILayout.EndHorizontal();
                        ...
                        EditorGUILayout.BeginHorizontal();
                        //GUILayout.Label("", GUILayout.Width(0.5f));
                        //for(int i=0;i<Texturelayer;i++)
                        int length = TexTexture.Length;
                        for (int i=0;i< length; i++)
                        {
                            string splat = "_Splat" + i;
                            if (GUILayout.Button((Texture)AssetDatabase.LoadAssetAtPath(T4MEditorFolder + "Img/up.png", typeof(Texture)), GUILayout.Width((340 - (length-1) * 3) / length)))
                            {
                                ...
                                if (PreceduralAdd)
                                {
                                    CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.SetTexture(splat, PreceduralAdd.GetTexture("_MainTex"));
                                }
                                else if (MaterialAdd)
                                {
                                    CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.SetTexture(splat, MaterialAdd);
                                }
                                selProcedural = 0;
                                PreceduralAdd = null;
                                MaterialAdd = null;
...
#if false
                        if (GUILayout.Button((Texture)AssetDatabase.LoadAssetAtPath(T4MEditorFolder + "Img/up.png", typeof(Texture))))
                        {
                            if (!PreceduralAdd && !MaterialAdd && Precedural)
                                PreceduralAdd = Precedural;
                        ...
                        if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_Splat6"))
                            if (GUILayout.Button((Texture)AssetDatabase.LoadAssetAtPath(T4MEditorFolder + "Img/up.png", typeof(Texture))))
                            {
                                ...
                                if (PreceduralAdd)
                                {
                                    CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.SetTexture("_Splat6", PreceduralAdd.GetTexture("_MainTex"));
                                }
                                else if (MaterialAdd)
                                {
                                    CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.SetTexture("_Splat6", MaterialAdd);
                                }
                                selProcedural = 6;
                                PreceduralAdd = null;
                                MaterialAdd = null;
                        ...
                        if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_Splat7"))
                            if (GUILayout.Button((Texture)AssetDatabase.LoadAssetAtPath(T4MEditorFolder + "Img/up.png", typeof(Texture))))
                            {
                                ...
                                if (PreceduralAdd)
                                {
                                    CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.SetTexture("_Splat7", PreceduralAdd.GetTexture("_MainTex"));
                                }
                                else if (MaterialAdd)
                                {
                                    CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.SetTexture("_Splat7", MaterialAdd);
                                }
                                selProcedural = 7;
                                PreceduralAdd = null;
                                MaterialAdd = null;
        ...
        for(int i=0;i<Texturelayer;i++)
        {
            if (selProcedural == i)
            {
                if (Texture_Layers[i])
                {
                    ...
                    Texture_Layers[i] = EditorGUILayout.ObjectField(Texture_Layers[i], typeof(Texture2D), true, GUILayout.Width(75), GUILayout.Height(75)) as Texture;
                    CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.SetTexture("_Splat"+i.ToString(), Texture_Layers[i]);
//                     if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_BumpSplat0"))
//                     {
...
//                     }
        ...
        else if (selProcedural == 6)
        {
            if (Layer7)
            {
                ...
                GUILayout.BeginHorizontal("Box");
                GUILayout.Label((Texture)AssetDatabase.LoadAssetAtPath(T4MEditorFolder + "Img/TDiff.jpg", typeof(Texture)));
                Layer6 = EditorGUILayout.ObjectField(Layer7, typeof(Texture2D), true, GUILayout.Width(75), GUILayout.Height(75)) as Texture;
                CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.SetTexture("_Splat6", Layer7);
            }
        }
#endif
    ...
    void MyT4MApplyChange()
    {
        ...
        if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_Control3"))
        {
            ...
            if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.GetTexture("_Control") != null)
            {
                Control3 = (Texture)AssetDatabase.LoadAssetAtPath(T4MPrefabFolder + "Terrains/Texture/" + CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.GetTexture("_Control").name + "C3.png", typeof(Texture));
            }
            else Control3 = (Texture)AssetDatabase.LoadAssetAtPath(T4MPrefabFolder + "Terrains/Texture/" + CurrentSelect.gameObject.name + "C3.png", typeof(Texture));

            if (Control3 == null)
                CreateControl3Text();
            else CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.SetTexture("_Control3", Control3);
        }

        for (int i=0;i<Texturelayer;i++)
        {
            if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_Splat"+i.ToString()))
                CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.SetTexture("_Splat" + i.ToString(), Texture_Layers[i]);
        }
        ...
        if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_Splat7"))
        {
            CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.SetTexture("_Splat7", Layer8);
        }
#endif
        if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_BumpSplat0"))
        {
    ...
    {
        Texture2D Control2 = new Texture2D(CotnrolTextureSize, CotnrolTextureSize, TextureFormat.ARGB32, true);
        Color[] ColorBase = new Color[CotnrolTextureSize * CotnrolTextureSize];
        ...
        TextureI.alphaIsTransparency = false;
    ...
    void CreateControl3Text()
    {
        Texture2D Control3 = new Texture2D(CotnrolTextureSize, CotnrolTextureSize, TextureFormat.ARGB32, true);
        Color[] ColorBase = new Color[CotnrolTextureSize * CotnrolTextureSize];
        for (var t = 0; t < ColorBase.Length; t++)
        {
            ColorBase[t] = new Color(0, 0, 0, 0);
        ...
        Control3.SetPixels(ColorBase);
        string path;
        if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.GetTexture("_Control") != null)
        {
            path = T4MPrefabFolder + "Terrains/Texture/" + CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.GetTexture("_Control").name + "C3.png";

        }
        else path = T4MPrefabFolder + "Terrains/Texture/" + CurrentSelect.gameObject.name + "C3.png";
        byte[] data = Control3.EncodeToPNG();
        File.WriteAllBytes(path, data);
        AssetDatabase.ImportAsset(path, ImportAssetOptions.ForceUpdate);
        ...
        Texture Contr3 = (Texture)AssetDatabase.LoadAssetAtPath(path, typeof(Texture));
        CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.SetTexture("_Control3", Contr3);
        IniNewSelect();
    }
    ...
    void ConvertUTerrain()
    {
        ...
        {
            Texture2D NewMaskText = new Texture2D(CotnrolTextureSize, CotnrolTextureSize, TextureFormat.ARGB32, true);
            Color[] ColorBase = new Color[CotnrolTextureSize * CotnrolTextureSize];
            for (var t = 0; t < ColorBase.Length; t++)
            {
    ...
    void Obj2T4M()
    {
        ...
        TextureIm.filterMode = FilterMode.Trilinear;
    ...
    void IniNewSelect()
    {
        ...
        if (CurrentSelect && CurrentSelect.GetComponent<T4MObjSC>() && CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial)
        {
            ...
            for(int i=0;i<Texturelayer;i++)
            {
                ...
                if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty(splat_layer))
                {
                    Texture_Layers[i] = CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.GetTexture(splat_layer);
                    LayerTiles[i] = CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.GetTextureScale(splat_layer);
                }
                else
                    Texture_Layers[i] = null;
            }
            ...
            if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_Splat6"))
            {
                Layer7 = CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.GetTexture("_Splat6");
                Layer7Tile = CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.GetTextureScale("_Splat6");
            }
            else Layer7 = null;

            if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_Splat7"))
            {
                Layer8 = CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.GetTexture("_Splat7");
                Layer8Tile = CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.GetTextureScale("_Splat7");
            }
            else Layer8 = null;
#endif
            if (CurrentSelect.gameObject.GetComponent<T4MObjSC>().T4MMaterial.HasProperty("_BumpSplat0"))
