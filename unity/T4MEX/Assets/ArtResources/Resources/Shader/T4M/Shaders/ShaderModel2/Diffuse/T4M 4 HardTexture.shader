...
#define Emissive(_D_var) _D_var.rgb +specularValue * _D_var.a
	float4 frag(v2f i) : COLOR
	{
		...
		/*		if (index.x != 0)
		{
		float4 channelA = tex2D(_Mask, lay4UV + coord1.xy);
		c.rgb = lerp(c.rgb, surf1.rgb, channelA.r);// *(1 - step(0, -index.x)));
		}*/

		if (index.y != 0)
		{
			fixed2 channel2 = fixed2(lay4UV.x + indexdiv4.y, lay4UV.y + indexmod4.y);
			fixed4 surf2 = tex2D(_Splat1, i.uv[0].xy);
			fixed4 channelB = tex2D(_Mask, channel2);
			c = lerp(c, surf2, channelB.r);
			//c.rgb = lerp(c.rgb, channelB.rgb, channelB.a * (1 - step(0, -index.y)));
		}

		if (index.z != 0)
		{
			fixed2 channel3 = fixed2(lay4UV.x + indexdiv4.z, lay4UV.y + indexmod4.z);
			fixed4 surf3 = tex2D(_Splat2, i.uv[0].xy);
			fixed4 channelC = tex2D(_Mask, channel3);
			c = lerp(c, surf3, channelC.g);
			//c.rgb = lerp(c.rgb, channelC.rgb, channelC.a * (1 - step(0, -index.z)));
		}

		if (index.w != 0)
		{
			fixed2 channel4 = fixed2(lay4UV.x + indexdiv4.w, lay4UV.y + indexmod4.w);
			fixed4 surf4 = tex2D(_Splat3, i.uv[0].xy);
			fixed4 channelD = tex2D(_Mask, channel4);
			c.rgb = lerp(c.rgb, surf4.rgb, channelD.g);
			//c.rgb = lerp(c.rgb, channelD.rgb, channelD.a * (1 - step(0, -index.w)));
		}

		if (index1.x != 0)
		{
			fixed2 channel5 = fixed2(lay4UV.x + indexdiv14.x, lay4UV.y + indexmod14.x);
			fixed4 surf5 = tex2D(_Splat4, i.uv[0].xy);
			fixed4 channelA = tex2D(_Mask, channel5);
			c = lerp(c, surf5, channelA.b);// *(1 - step(0, -index.x)));
		}

		if (index1.y != 0)
		{
			fixed2 channel6 = fixed2(lay4UV.x + indexdiv14.y, lay4UV.y + indexmod14.y);
			fixed4 surf6 = tex2D(_Splat5, i.uv[0].xy);
			fixed4 channelB = tex2D(_Mask, channel6);
			c = lerp(c, surf6, channelB.b);// *(1 - step(0, -index.x)));
		}
