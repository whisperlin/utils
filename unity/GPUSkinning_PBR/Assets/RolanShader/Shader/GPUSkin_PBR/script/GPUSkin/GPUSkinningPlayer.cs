...
{
    public delegate void OnAnimFrameEvent(GPUSkinningPlayer player, string clipName, int eventId);
    ...
    public event OnAnimFrameEvent OnAnimEventCallback;
    ...
    /// 当前正播放的动画，是否需要回调帧事件
    /// </summary>
    private bool mCurClipEventCb = false;

    /// <summary>
    /// CrossFade下一动画，是否需要回调帧事件
    /// </summary>
    private bool mCrossNextClipEventCb = false;
    ...
    public void Play(string clipName)
    {
        ...
        for(int i = 0; i < numClips; ++i)
        {
            if(clips[i].name == clipName)
            {
                ...
                {
                    mCurClipEventCb = true;
                    mCrossNextClipEventCb = false;
    ...
    {
        if (playingClip == null) { this.Play(clipName); return; }
        ...
        for (int i = 0; i < numClips; ++i)
        {
            if (clips[i].name == clipName)
            {
                if (playingClip != clips[i])
                {
                    ...
                    mCurClipEventCb = false;
                    mCrossNextClipEventCb = true;
                    SetNewPlayingClip(clips[i]);
                    return;
                }
    ...
    private void SetNewPlayingClip(GPUSkinningClip clip)
    {
        ...
        //OnEventEnter
        if (this.OnAnimEventCallback != null) {
            this.OnAnimEventCallback(this, clip.name, 0);
        }
    }
    ...
    {
        if (this.OnAnimEventCallback != null && clip != null
            && clip.events != null && clip.events.Length > 0)
        {
            ...
            {
                if (events[i].frameIndex <= frameIndex && mCurClipEventCb)
                {
                    mCurClipEventCb = false;
                    OnAnimEventCallback(this, clip.name, events[i].eventId); break;
                }
            }
        }
    }
    ...
    private void UpdateMaterial(float deltaTime, GPUSkinningMaterial currMtrl)
    {
        ...
        if(lastPlayingClip == playingClip && lastPlayingFrameIndex == frameIndex)
        {
            ...
            if (OnAnimEventCallback != null && this.NormalizedTime == 1f)
            {
                this.OnAnimEventCallback(this, playingClip.name, -1);
            }
            return;
        }
        ...
        else
        {
            if (!mCurClipEventCb)
            {
                mCurClipEventCb = mCrossNextClipEventCb == true;
                mCrossNextClipEventCb = false;
            }
        }
